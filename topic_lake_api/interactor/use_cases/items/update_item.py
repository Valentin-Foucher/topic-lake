...
class UpdateItem(UseCase):
    ...
    async def execute(self, item_id: int, content: str, rank: int):
        item = await self._items_repository.get(item_id)
        if not item:
            raise DoesNotExist(f'Item {item_id} does not exist')

        await self._items_repository.update_ranks_for_topic(item.topic_id, rank)
        return await self._items_repository.update(
            item_id,
            content,
            await determine_rank(self._items_repository, rank, item.topic_id)
        )
